...
> 本文件记录的是 AI 能力移除前的扩展截图计划，其中「智能问答」「图片诊断」相关条目对应的模块已从当前工作区移除（Schema 6），保留的业务能力只剩「今日农活」与「统计面板」。重新申报前需按当前功能重写本计划。

本清单用于本轮新增功能的软著展示准备，不改变已封存的 V1.0 最终交付材料。截图应来自本地演示数据或 Mock 数据，截图前检查页面中不存在 Token、Cookie、用户 ID、数据库路径、Request ID 或内部错误详情。
