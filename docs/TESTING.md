...
- 后端：认证与 Token 摘要、今日农活规则、农活记录落库与统计接口、Schema 迁移（含 v5→v6 删除 AI 表）、备份恢复、配置解析、限流、错误载荷和 SQLite 并发写入。
- 前端：接口错误映射、版本号格式化，以及 `e2e/decision.spec.ts` 的 6 条流程，覆盖访客登录、生成今日农活建议、统计面板记录与趋势、帮助与关于软件、移动端导航和不提供个人 API Key 入口。
