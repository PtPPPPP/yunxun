...
- 核对当前代码支持的 Schema（当前为 9）；跨大版本升级前必须确认目标数据库已备份，Schema 6 会删除会话、消息和幂等表以及用户模型偏好字段。
...
- 部署前端，执行登录、地块新建（自动建第一茬）、结束本茬并开始新茬、台账记一次作业（含采收产量与待办）、今日农活建议生成和统计面板冒烟。

## 发布后

- 检查请求 ID、错误日志和数据库 Schema。
- 验证登录、地块与茬次增删与换茬、台账增删、待办完成与重开、生成建议、统计面板记录和定时备份。
