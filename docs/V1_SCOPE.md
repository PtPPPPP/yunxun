...
正式基线为 `v1.0.0` / `b952778`，机器版本 `1.0.0`，文案版本 `V1.0.0`。当前工作区数据库为 Schema 7。

正式能力：注册登录、访客、地块档案、农事台账（含用量与费用）、今日农活建议、使用统计面板、SQLite 持久化、Bearer 认证和基础安全控制。所有计算与建议由本机规则引擎完成，不访问任何外部服务。
