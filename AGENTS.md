...
保留并维护的能力：认证（注册/登录/访客/退出）、地块与茬次档案、农事台账（含采收产量与费用）、农事待办、今日农活建议、按茬次的投入产出核算、统计面板、数据库迁移与备份、发布脚本与 CI 门禁。

业务主体是 `plots`（地块），当季作物与起止日期由 `plot_seasons`（茬次，Schema 9）承载，**不要再往地块上加作物字段**——那会和茬次表形成第二个真相来源。台账 `farm_records`（Schema 7）与待办 `farm_tasks`（Schema 8）挂在地块下。新增业务功能应优先挂靠地块，不要另起一个与地块平行的孤立实体；确实是杂事、不针对具体地块的（如「去镇上买地膜」）才允许不关联地块。

界面上面向的是农户：环境名、后端地址、限流值、数据库路径这类给部署者看的信息不要放进页面（变量名放进启动日志）。
