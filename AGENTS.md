...
保留并维护的能力：认证（注册/登录/访客/退出）、地块档案、农事台账（含采收产量与费用）、农事待办、今日农活建议、投入产出核算、统计面板、数据库迁移与备份、发布脚本与 CI 门禁。

业务主体是 `plots`（地块）与挂在它下面的 `farm_records`（农事台账，Schema 7）和 `farm_tasks`（待办，Schema 8）。新增业务功能应优先挂靠地块，不要另起一个与地块平行的孤立实体；确实是杂事、不针对具体地块的（如「去镇上买地膜」）才允许不关联地块。
