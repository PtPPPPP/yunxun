...
## Corpus Check
- 99 files · ~38,498 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 780 nodes · 1748 edges · 61 communities (35 shown, 26 thin omitted)
...
- `main()` --calls--> `init_db()`  [EXTRACTED]
  scripts/check_release.py → backend/app/core/database.py
...
### Community 0 - "App.tsx"
Cohesion: 0.06
Nodes (56): App(), DecisionWorkspace, VisionWorkspace, AuthScreen(), AuthScreenProps, ChatWorkspace(), ChatWorkspaceProps, prompts (+48 more)

### Community 1 - "auth.py"
Cohesion: 0.34
Nodes (7): create_backup(), Path, restore_backup(), validate_database(), BackupRestoreTestCase, main(), main()

### Community 2 - "chat.py"
Cohesion: 0.06
Nodes (65): get_current_user(), auth_me_api(), auth_profile_api(), guest_login_api(), _limit_auth(), login_api(), logout_api(), Request (+57 more)

### Community 3 - "get_settings"
Cohesion: 0.09
Nodes (37): _format_bounds(), get_settings(), _getenv(), _parse_bool(), _parse_float(), _parse_int(), _parse_optional_str(), _resolve_database_path() (+29 more)

### Community 4 - "chat.py"
Cohesion: 0.08
Nodes (54): _apply_schema_v1(), _create_auth_tokens_table(), _ensure_auth_tokens_schema(), ensure_parent_dir(), get_connection(), get_db_path(), init_db(), migrate_schema() (+46 more)

### Community 5 - "Settings"
Cohesion: 0.10
Nodes (14): has_real_api_key(), _normalize_example_key(), _parse_csv(), Settings, validate_startup_settings(), build_runtime_status(), build_runtime_warnings(), Any (+6 more)
...
Cohesion: 0.07
Nodes (13): DatabaseIdempotencyStore, IdempotencyClaim, Any, datetime, AssistantFaultTestCase, ModelError, Exception, DatabaseIdempotencyStoreTestCase (+5 more)
...
Cohesion: 0.13
Nodes (23): liveness_api(), readiness_api(), error_response(), http_exception_handler(), Exception, HTTPException, Request, 构造统一的错误响应。      ``code`` 仅在 ``AppError``（或显式传入）时出现，普通错误载荷保持     ``{"success": Fa (+15 more)
...
### Community 58 - "release_rehearsal.py"
Cohesion: 0.44
Nodes (9): Popen, api(), main(), Path, run(), running_backend(), start_backend(), stop_backend() (+1 more)

### Community 59 - "allowed"
Cohesion: 0.83
Nodes (3): allowed(), main(), Path
...
- **Why does `Settings` connect `Settings` to `chat.py`, `get_settings`, `chat.py`, `DatabaseIdempotencyStore`, `AppError`, `main.py`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `get_settings()` connect `get_settings` to `auth.py`, `chat.py`, `chat.py`, `Settings`, `AppError`, `main.py`?**
