...
class DatabaseMigrationTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.patcher = patch("backend.app.core.database.get_settings", return_value=make_settings(self.db_path))
        self.patcher.start()
    ...
    def test_empty_database_reaches_current_version_without_ai_objects(self) -> None:
        ...
        self.assertTrue(
            {"users", "auth_tokens", "tool_records", "plots", "plot_seasons", "farm_records", "farm_tasks"} <= tables
        )
        self.assertFalse(
