...
云寻是面向田块管理的农业工作台：登记地块档案并按**茬次**记录每季作物，按地块记录每次农事作业（含用量、费用、采收产量与单价），用待办排好接下来的活，并结合作物、生长期、降雨概率和土壤墒情生成当天可执行的建议。打药记录会按安全间隔期给出最早采收日，统计面板按茬次算出投入产出，便于跨年对比。全部计算在本地规则引擎完成，不依赖任何外部服务。
