...
```powershell
$ProjectPath = "D:\Program\vscode\yunxun"
Set-Location "D:\Program\vscode\yunxun\frontend"
npm install
npm run dev
```
...
3. 站点地址：`https://PtPPPPP.github.io/work/`。
...
`frontend/vite.config.ts` 已在构建时设置 `base: "/work/"` 以匹配项目站点路径；本地 `npm run dev` 仍使用根路径 `/`。
...
注意：Render 免费层文件系统不持久，SQLite 数据会在重启或重新部署后重置；免费层 15 分钟无访问会休眠，冷启动约 30s。需要持久化数据请升级数据库或换用带持久磁盘的平台（如 Fly.io 的 volume）。
